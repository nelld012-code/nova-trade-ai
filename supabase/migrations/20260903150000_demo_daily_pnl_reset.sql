...
-- This keeps the daily loss limit scoped to the current simulation day without
-- introducing client-controlled accounting fields.
...
BEGIN
  ...
  IF v_closed_id IS NOT NULL THEN
    ...
    UPDATE public.portfolio SET balance=greatest(0,coalesce(balance,0)+v_pnl), today_pnl=v_today_pnl, total_pnl=v_total_pnl,
      performance_pct=CASE WHEN coalesce(total_deposited,0)>0 THEN (v_total_pnl/total_deposited)*100 ELSE 0 END, updated_at=v_now WHERE user_id=v_user;
    v_open_count := greatest(0,v_open_count-1);
  END IF;

  IF v_open_count < v_risk.max_open_positions AND v_today_pnl > -abs(v_risk.max_daily_loss_usd) THEN
    ...
    IF v_size > 0 THEN
      INSERT INTO public.operations(user_id,asset,direction,entry_price,size,pnl,return_pct,status,opened_at) VALUES(v_user,v_asset,v_direction,v_entry,v_size,0,0,'OPEN',v_now) RETURNING id INTO v_opened_id;
  ...
  RETURN jsonb_build_object('ok',true,'mode','DEMO','closed_operation_id',v_closed_id,'opened_operation_id',v_opened_id,
    'realized_pnl',round(coalesce(v_pnl,0),2),'today_pnl',round(coalesce(v_today_pnl,0),2),
