-- DEMO safety maintenance: reset today's P&L at the first execution cycle of a new UTC day.
-- The execution remains DEMO-only and keeps all accounting/risk decisions server-side.
...
BEGIN
  ...
  -- A portfolio update from a prior calendar day starts a fresh daily P&L window.
  v_today_pnl := CASE
  ...
  IF v_closed_id IS NOT NULL THEN
    ...
    v_move := (random() - 0.48) * CASE upper(coalesce(v_robot.risk_level, 'MEDIUM'))
      ...
      ELSE 0.010
    END;
    v_exit := round((v_entry * (1 + CASE
      WHEN upper(v_direction) IN ('LONG','BUY') THEN v_move
      ELSE -v_move
    END))::numeric, 8);
    v_return_pct := CASE
    ...
    -- balance represents total equity; invested represents current open exposure.
  ...
  -- Never allocate more than the configured risk cap, robot allocation,
  -- or the portfolio's uncommitted exposure. This prevents the previous
  -- minimum-size rule from exceeding max_position_usd on small accounts.
  v_available_exposure := greatest(0, coalesce(v_portfolio.balance,0) - coalesce(v_portfolio.invested,0));
  v_max_size := least(
    abs(v_risk.max_position_usd),
    greatest(0,coalesce(v_robot.capital_allocation,0)),
    v_available_exposure
  );
