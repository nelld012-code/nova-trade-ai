...
BEGIN
  ...
  IF v_closed_id IS NOT NULL THEN
    ...
    INSERT INTO public.demo_market_state(user_id, asset, price)
    VALUES (v_user, v_asset, v_base)
    ON CONFLICT (user_id, asset) DO NOTHING;

    SELECT price INTO v_price
    FROM public.demo_market_state
    ...
    UPDATE public.demo_market_state
    SET price = v_price, updated_at = v_now
    WHERE user_id = v_user AND asset = v_asset;

    v_exit := v_price;
    v_return_pct := CASE
      WHEN upper(v_direction) IN ('LONG','BUY') THEN ((v_exit-v_entry)/NULLIF(v_entry,0))*100
      ELSE ((v_entry-v_exit)/NULLIF(v_entry,0))*100
    END;
    v_pnl := round(v_size * v_return_pct / 100, 2);

    UPDATE public.operations
    SET exit_price=v_exit, pnl=v_pnl, return_pct=v_return_pct, status='CLOSED', closed_at=v_now
    ...
    SET balance=greatest(0,coalesce(balance,0)+v_pnl),
        ...
        invested=(SELECT coalesce(sum(size),0) FROM public.operations WHERE user_id=v_user AND upper(status)='OPEN'),
        updated_at=v_now
    WHERE user_id=v_user;
    v_open_count := greatest(0,v_open_count-1);
  END IF;

  -- Open a new position only within all configured exposure limits.
  SELECT balance, invested INTO v_portfolio.balance, v_portfolio.invested
  FROM public.portfolio WHERE user_id=v_user FOR UPDATE;
  ...
  v_size_multiplier := CASE upper(coalesce(v_robot.risk_level,'MEDIUM'))
    WHEN 'LOW' THEN 0.35 WHEN 'BAJO' THEN 0.35 WHEN 'HIGH' THEN 0.80 WHEN 'ALTO' THEN 0.80 ELSE 0.55
  END;
  v_size := round(least(v_max_size, v_max_size * v_size_multiplier)::numeric,2);

  IF v_open_count < v_risk.max_open_positions
     AND v_today_pnl > -abs(v_risk.max_daily_loss_usd)
     AND v_size > 0 THEN
    ...
    UPDATE public.portfolio
    SET invested=(SELECT coalesce(sum(size),0) FROM public.operations WHERE user_id=v_user AND upper(status)='OPEN'), updated_at=v_now
    WHERE user_id=v_user;
  END IF;
  ...
  VALUES (
    v_user,
    greatest(0,coalesce(v_portfolio.balance,0)),
    v_today_pnl,
    v_total_pnl,
    greatest(0,coalesce(v_portfolio.invested,0)),
    (SELECT count(*) FROM public.operations WHERE user_id=v_user AND upper(status)='OPEN'),
    v_now
  );
  ...
  RETURN jsonb_build_object(
    ...
    'today_pnl',round(v_today_pnl,2),
    'total_pnl',round(v_total_pnl,2),
    'equity',round(greatest(0,coalesce(v_portfolio.balance,0)),2),
