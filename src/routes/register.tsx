...
function RegisterPage() {
  ...
  async function handleRegister(event: React.FormEvent) {
    ...
    if (!dateOfBirth) { setError("La fecha de nacimiento es obligatoria."); return; }
    if (calculateAge(dateOfBirth) < 18) { setError("Debes tener al menos 18 años para crear una cuenta."); return; }
    if (!accepted) { setError("Debes aceptar los términos y condiciones para continuar."); return; }
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          ...
          phone: phone.trim(),
          country,
          country_name: selectedCountry?.name ?? country,
  ...
  return (
    <AuthShell title="Crea tu cuenta" description="Completa tus datos para comenzar tu experiencia con TradeNova AI.">
