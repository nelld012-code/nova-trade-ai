...
function PortfolioPage() {
  ...
  useEffect(() => { let active = true; supabase.from("portfolio").select("balance,invested,performance_pct,today_pnl,total_deposited,total_pnl").eq("user_id", session.user.id).maybeSingle().then(({ data }) => { if (active && data) setPortfolio(data); }); return () => { active = false; }; }, [session.user.id]);
  return <DashboardPageLayout title="Portafolio" description="Visualiza tu capital, rendimiento y resultados acumulados."><PortfolioPanel portfolio={portfolio} /></DashboardPageLayout>;
}
