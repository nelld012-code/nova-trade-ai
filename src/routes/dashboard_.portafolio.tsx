...
function PortfolioPage() { const { session } = Route.useRouteContext(); const [portfolio, setPortfolio] = useState<Portfolio>(demoPortfolio); useEffect(() => { let active = true; supabase.from("portfolio").select("balance,invested,performance_pct,today_pnl,total_deposited,total_pnl").eq("user_id", session.user.id).maybeSingle().then(({ data }) => { if (active && data) setPortfolio(data); }); return () => { active = false; }; }, [session.user.id]); return <DashboardPageLayout title={{ es:"Portafolio", en:"Portfolio" }} description={{ es:"Visualiza tu capital, rendimiento y resultados acumulados.", en:"View your capital, performance, and accumulated results." }}><PortfolioPanel portfolio={portfolio} /></DashboardPageLayout>; }
