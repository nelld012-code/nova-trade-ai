...
function DashboardPage() {
  ...
  return <DashboardShell email={session.user.email ?? null} userName={name} avatarUrl={avatarUrl} countryCode={country}>
    <section className="mx-auto max-w-7xl space-y-6">
      ...
      <Card className="border-amber-200 bg-amber-50/50 shadow-sm"><CardContent className="flex flex-col gap-2 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">{hasPortfolio ? (en ? "Connected portfolio" : "Portafolio conectado") : (en ? "No portfolio yet" : "Aún no hay portafolio")}</p><p className="text-sm text-slate-600">{hasPortfolio ? (en ? "The indicators come from your portfolio record in Supabase." : "Los indicadores proceden de tu registro de portafolio en Supabase.") : (en ? "No financial values are fabricated. Create or fund a portfolio to see real account data here." : "No se muestran valores financieros inventados. Crea o fondea un portafolio para ver aquí los datos reales de tu cuenta.")}</p></div><span className="text-xs font-semibold uppercase tracking-wide text-amber-700">{hasPortfolio ? "Supabase" : (en ? "Empty" : "Vacío")}</span></CardContent></Card>
    </section>
  </DashboardShell>;
}
