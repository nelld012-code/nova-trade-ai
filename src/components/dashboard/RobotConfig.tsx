...
export function RobotConfig({ userId }: { userId: string }) {
  ...
  return (
    <Card id="configurar-robot" className="border-slate-200 shadow-sm">
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">Activar robot</p><p className="text-xs text-slate-500">Solo se ejecuta como simulación mientras esté en DEMO.</p></div><Switch checked={enabled} disabled={saving} onCheckedChange={(value) => { setEnabled(value); void saveRobot(value); }} /></div>

        <div className="flex flex-col gap-3 sm:flex-row sm:items-center"><Button onClick={() => void saveRobot()} disabled={saving} className="gap-2"><Save className="h-4 w-4" /> {saving ? <><Loader2 className="h-4 w-4 animate-spin" /> Guardando...</> : "Guardar configuración"}</Button><Button variant="outline" onClick={() => void saveRobot(true)} disabled={saving} className="gap-2"><Play className="h-4 w-4" /> Activar DEMO</Button><Button variant="outline" onClick={() => void saveRobot(false)} disabled={saving} className="gap-2"><Square className="h-4 w-4" /> Detener</Button></div>
        {message && <p className="text-sm text-slate-600">{message}</p>}
        {mode === "LIVE" && <div className="flex gap-2 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700"><ShieldAlert className="mt-0.5 h-4 w-4 shrink-0" /><span>El modo LIVE requiere una implementación de ejecución real y controles adicionales. Esta versión no ejecuta operaciones reales.</span></div>}
      </CardContent>
    </Card>
  );
}
