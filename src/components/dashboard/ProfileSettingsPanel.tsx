...
type Settings = {
  language: string; currency: string; theme: string; email_notifications: boolean;
  push_notifications: boolean; risk_alerts: boolean; two_factor_enabled: boolean;
};
...
function calculateAge(dateOfBirth: string) {
  const today = new Date(); const birth = new Date(`${dateOfBirth}T00:00:00`);
  let age = today.getFullYear() - birth.getFullYear(); const month = today.getMonth() - birth.getMonth();
  if (month < 0 || (month === 0 && today.getDate() < birth.getDate())) age--; return age;
}
...
export function ProfileSettingsPanel({ userId, email }: { userId: string; email?: string | null }) {
  ...
  const [dateOfBirth, setDateOfBirth] = useState(""); const [avatarUrl, setAvatarUrl] = useState<string | null>(null);
  const [settings, setSettings] = useState<Settings>(defaultSettings); const [newPassword, setNewPassword] = useState("");
  const [confirmPassword, setConfirmPassword] = useState(""); const [savingProfile, setSavingProfile] = useState(false);
  const [uploadingAvatar, setUploadingAvatar] = useState(false); const [savingSettings, setSavingSettings] = useState(false);
  const [changingPassword, setChangingPassword] = useState(false); const [message, setMessage] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: profile }, { data: saved }] = await Promise.all([
        db.from("profiles").select("full_name,phone,country,address,date_of_birth,avatar_url").eq("id", userId).maybeSingle(),
        db.from("settings").select("language,currency,theme,email_notifications,push_notifications,risk_alerts,two_factor_enabled").eq("user_id", userId).maybeSingle(),
      ]);
      if (!active) return;
      if (profile) { setFullName(profile.full_name ?? ""); setPhone(profile.phone ?? ""); setCountry(profile.country ?? ""); setAddress(profile.address ?? ""); setDateOfBirth(profile.date_of_birth ?? ""); setAvatarUrl(profile.avatar_url ?? null); }
  ...
  async function uploadAvatar(file: File) {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("avatars").upload(path, file, { cacheControl: "3600", upsert: true, contentType: file.type });
      if (uploadError) throw uploadError;
      const { data } = supabase.storage.from("avatars").getPublicUrl(path);
      const db = supabase as any;
      const { error: saveError } = await db.from("profiles").update({ avatar_url: data.publicUrl }).eq("id", userId);
      if (saveError) throw saveError;
      setAvatarUrl(data.publicUrl); setMessage("Foto de perfil actualizada correctamente.");
  ...
  async function saveProfile() {
    ...
    if (!dateOfBirth || calculateAge(dateOfBirth) < 18) { setError("El perfil requiere una fecha de nacimiento válida de una persona mayor de 18 años."); return; }
    setSavingProfile(true);
    const db = supabase as any;
    const { error: saveError } = await db.from("profiles").upsert({ id: userId, full_name: fullName.trim(), email: email ?? "", phone: phone.trim() || null, country, address: address.trim() || null, date_of_birth: dateOfBirth });
    setSavingProfile(false);
    if (saveError) setError(saveError.message); else setMessage("Perfil actualizado correctamente.");
  }

  async function saveSettings() {
    clearStatus(); setSavingSettings(true);
    const { error: saveError } = await supabase.from("settings").upsert({ user_id: userId, ...settings });
    setSavingSettings(false); if (saveError) setError(saveError.message); else setMessage("Preferencias guardadas correctamente.");
  }
  ...
  async function changePassword() {
    ...
    setChangingPassword(true); const { error: passwordError } = await supabase.auth.updateUser({ password: newPassword }); setChangingPassword(false);
    if (passwordError) setError(passwordError.message); else { setNewPassword(""); setConfirmPassword(""); setMessage("Contraseña actualizada correctamente."); }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card id="perfil" className="border-slate-200 shadow-sm">
        <CardHeader><div className="flex items-center gap-3"><div className="rounded-xl bg-blue-50 p-2 text-blue-600"><UserRound className="h-5 w-5" /></div><div><CardTitle>Perfil personal</CardTitle><CardDescription>Actualiza tus datos y foto de perfil.</CardDescription></div></div></CardHeader>
        <CardContent className="space-y-6">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
            <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-full border-4 border-white bg-slate-100 shadow ring-1 ring-slate-200">
              {avatarUrl ? <img src={avatarUrl} alt={`Foto de ${fullName || "usuario"}`} className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-2xl font-bold text-slate-400">{fullName.trim().charAt(0).toUpperCase() || "U"}</div>}
            </div>
            <div><p className="font-semibold">Foto de perfil</p><p className="mb-3 text-xs text-slate-500">JPG, PNG o WebP · máximo 5 MB.</p><label className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-medium shadow-sm hover:bg-slate-50"><Camera className="h-4 w-4" />{uploadingAvatar ? "Subiendo..." : "Cambiar foto"}<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" disabled={uploadingAvatar} onChange={(e) => { const file = e.target.files?.[0]; if (file) void uploadAvatar(file); e.currentTarget.value = ""; }} /></label></div>
          </div>
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="space-y-2"><label className="text-sm font-medium">Teléfono</label><Input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+58 ..." /></div>
            <div className="space-y-2"><label className="text-sm font-medium">País</label><div className="relative"><Globe2 className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><select value={country} onChange={(e) => setCountry(e.target.value)} className="h-10 w-full rounded-md border border-input bg-background pl-9 pr-3 text-sm"><option value="">Selecciona tu país</option>{countryOptions.map((item) => <option key={item.code} value={item.code}>{item.flag} {item.name}</option>)}</select></div></div>
            <div className="space-y-2 sm:col-span-2"><label className="text-sm font-medium">Dirección</label><div className="relative"><MapPin className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><Input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Calle, número, ciudad, estado..." className="pl-9" /></div></div>
            <div className="space-y-2"><label className="text-sm font-medium">Fecha de nacimiento</label><div className="relative"><CalendarDays className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><Input type="date" value={dateOfBirth} max={maxBirthDate} onChange={(e) => setDateOfBirth(e.target.value)} className="pl-9" /><p className="text-xs text-slate-500">Solo mayores de 18 años.</p></div></div>
