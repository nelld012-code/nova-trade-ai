...
const navItems = [
  { label: "Dashboard", href: "/dashboard", icon: LayoutDashboard }, { label: "Robot IA", href: "/dashboard#robot", icon: Bot },
  { label: "Operaciones", href: "/dashboard#operaciones", icon: CircleDollarSign }, { label: "Portafolio", href: "/dashboard#portafolio", icon: Wallet },
  { label: "Notificaciones", href: "/dashboard#notificaciones", icon: Bell }, { label: "Perfil", href: "/dashboard#perfil", icon: User },
  { label: "Preferencias", href: "/dashboard#preferencias", icon: Settings }, { label: "Seguridad", href: "/dashboard#seguridad", icon: ShieldCheck },
];
...
export function DashboardShell({ children, email, userName, avatarUrl, countryCode }: { children: ReactNode; email?: string | null; userName?: string; avatarUrl?: string | null; countryCode?: string }) {
  ...
  const UserIdentity = ({ mobile = false }: { mobile?: boolean }) => <div className={`flex items-center gap-3 ${mobile ? "px-2 py-3" : ""}`}>
    <div className="h-9 w-9 shrink-0 overflow-hidden rounded-full border border-slate-200 bg-slate-100">{avatarUrl ? <img src={avatarUrl} alt={`Foto de ${userName || "usuario"}`} className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-sm font-bold text-slate-500">{initials}</div>}</div>
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-950">
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-950 text-white transition-transform lg:hidden ${open ? "translate-x-0" : "-translate-x-full"}`}><div className="flex items-center justify-between border-b border-white/10 p-5"><div className="font-black">TRADE NOVA AI</div><Button size="icon" variant="ghost" className="text-white hover:bg-white/10" onClick={() => setOpen(false)}><X /></Button></div><nav className="space-y-1 p-3">{navItems.map((item) => { const Icon = item.icon; return <a key={item.label} href={item.href} onClick={() => setOpen(false)} className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm text-slate-300 hover:bg-white/10 hover:text-white"><Icon className="h-4 w-4" />{item.label}</a>; })}</nav><div className="absolute bottom-4 left-4 right-4"><UserIdentity mobile /><Button variant="ghost" className="w-full justify-start gap-3 text-slate-300 hover:bg-white/10 hover:text-white" onClick={logout}><LogOut className="h-4 w-4" /> Cerrar sesión</Button></div></aside>
