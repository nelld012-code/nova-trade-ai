...
type TickResult = { ok?: boolean; mode?: string; realized_pnl?: number; today_pnl?: number; open_positions?: number; max_open_positions?: number; opened_operation_id?: string | null; closed_operation_id?: string | null };
...
export function DemoExecutionPanel({ userId: _userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    if (autoRun) timerRef.current = setInterval(() => void execute(), Number(intervalSeconds) * 1000);
  ...
  return <Card className="border-blue-200 bg-blue-50/40 shadow-sm">
    <CardHeader><div className="flex items-start justify-between gap-4"><div><CardTitle className="flex items-center gap-2"><Activity className="h-5 w-5 text-blue-600" />{en ? "DEMO execution engine" : "Motor de ejecución DEMO"}</CardTitle><CardDescription>{en ? "Controlled simulation cycles update Operations and Portfolio atomically." : "Los ciclos de simulación controlados actualizan Operaciones y Portafolio de forma atómica."}</CardDescription></div><Badge variant="outline" className={autoRun ? "border-emerald-300 text-emerald-700" : "border-blue-300 text-blue-700"}>{autoRun ? (en ? "RUNNING" : "ACTIVO") : "DEMO"}</Badge></div></CardHeader>
    <CardContent className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 text-xs text-slate-500"><span className="rounded-full border bg-white px-2.5 py-1">{en ? "Cycles" : "Ciclos"}: {cycleCount}</span><span className="rounded-full border bg-white px-2.5 py-1">{en ? "Interval" : "Intervalo"}: {intervalSeconds}s</span></div>
      ...
      {result && <div className="flex flex-wrap gap-2 text-xs"><Badge variant="secondary">{result.opened_operation_id ? (en ? "Position opened" : "Posición abierta") : (en ? "No new position" : "Sin nueva posición")}</Badge><Badge variant="secondary">{result.closed_operation_id ? (en ? "Position settled" : "Posición liquidada") : (en ? "No settlement" : "Sin liquidación")}</Badge></div>}
