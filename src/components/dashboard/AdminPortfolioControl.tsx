...
import { Button } from "@/components/ui/button";
 ...
 export function AdminPortfolioControl({ userId }: { userId: string }) {
   ...
   useEffect(() => {
     ...
     async function loadPortfolio() {
       ...
       setValues(data ? {
         ...
         invested: String(data.invested ?? 0),
         total_deposited: String(data.total_deposited ?? 0),
         today_pnl: String(data.today_pnl ?? 0),
         total_pnl: String(data.total_pnl ?? 0),
         performance_pct: String(data.performance_pct ?? 0),
       } : emptyValues);
     }
     void loadPortfolio();
     return () => { active = false; };
   ...
   async function saveValues() {
     ...
     const parsed = Object.fromEntries(Object.entries(values).map(([key, value]) => [key, numberValue(value)]));
     if (Object.values(parsed).some((value) => typeof value !== "number" || !Number.isFinite(value))) { setError("Todos los valores deben ser números válidos."); return; }
     if (Object.values(parsed).some((value) => (value as number) < 0) && !["today_pnl", "total_pnl"].some((key) => (parsed as any)[key] < 0)) { setError("Los valores de capital no pueden ser negativos."); return; }
     setSaving(true);
     const db = supabase as any;
     ...
     else setMessage("Valores del dashboard actualizados correctamente.");
   }
   ...
   return <Card id="administracion" className="border-blue-200 bg-blue-50/30 shadow-sm">
     <CardHeader>
       <div className="flex items-center gap-3">
         <div className="rounded-xl bg-blue-600 p-2 text-white"><ShieldCheck className="h-5 w-5" /></div>
         <div><CardTitle>Administración · Valores del dashboard</CardTitle><CardDescription>Solo visible para administradores. Puedes modificar los indicadores financieros de cada usuario.</CardDescription></div>
       </div>
     </CardHeader>
